...
#maybe introduce that this is an r markdown file which is different than writing an r script file 
#could also add an image of the Rstuido interface highliting the sections for you command line, scripts, working space/environment, and directory sections, etc.

#maybe also have the students set their directory and talk about the importance of your working directory

```{r, echo= TRUE}
#do you need to also add install.packages or bioc::install type of lines for people's first time installing packges?
...
#which versions of R and R_studio are you recommending?
...
#maybe include another line of code that utilizes one of the variables within a new function or calculation to showcase you stared variables can be immplemented in downsteam analysis/code
```
...
#the phrase "unless it is a compiled version" might be a little confusing or overwhelming. Maybe add a section that has ls() and ls to force students to "run" both commands and see the differences
...
#this could also be an opportunity to share some rdocumentation links that showcase what students may or may not google (could always use ? but I actually prefer pulling it up on a separate winoww) that give examples of the different inputs and outputs a function may have
...
Word of Caution cont'd:  Very nice and apparently pretty close to 3.1416 so that they could get work done. Now, as we all know, the value of pi is the ratio of a circle’s circumference to its diameter. We also know that it is a transcendental number (e.g., on that cannot be produced using finite algebraic operations) and its decimal values never repeat.
```{r, echo=TRUE}
...
```
#do not have "constants_table.png" file 
...
Most factors are given in as characters so that naming isn’t a problem. Below is an example of five observations where the categorical variable sex of the organism is recorded.
...
#c for concatenate? is it combine? Idk
...
#first digit denoting rows and second denoting columns
...
#do you want to show examples of using a tibble and taking advantage of its capabilities??
